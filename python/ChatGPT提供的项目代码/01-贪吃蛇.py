...
def draw_sanke(snake_block_size, snake_list):
    for x in snake_list:
...
def game_loop():
    ...
    # 随机产生食物的位置
    foodx = random(random.randrange(0,width - snake_block_size)/10.0) * 10.0
    foody = random(random.randrange(0,height- snake_block_size)/10.0) * 10.0
    ...
    while not game_over:
        ...
        draw_sanke(snake_block_size, snake_List)
        pygame.display.update()
