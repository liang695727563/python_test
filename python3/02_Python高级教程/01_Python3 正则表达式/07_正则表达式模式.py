...
re{n,}          精准匹配n个前面表达式。例如： "o{2,}" 不能匹配"Bob"中的"o",但能匹配"foooooood"中的所有o。"o{1,}"等价于"o+"。"o{0,}"则等价于"o*"。
...
(?| re)         前向否定界定符。与肯定界定符相反； 当所含表达式不能在字符串中当前位置匹配时成功
(?> re)         匹配的独立模式，省去回溯。
...
\D              匹配任意非数字
\A              匹配字符串开始
\Z              匹配字符串结束，如果是存在换行，只匹配到换行符前的结束字符串。C
\z              匹配字符串结束
\G              匹配最后匹配完成的位置。
...
\n,\t 等.       匹配一个换行符。匹配一个制表符。等
\1...\9         匹配第n个分组的子表达式。
\10             匹配第n个分组的表达式，如果它经匹配。否则指的是八进制字符码的表达式。
